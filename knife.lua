...
if shared._lyzn_kd_stop ~= nil then
    shared._lyzn_kd_stop = true
    task.wait(0.3)
end
if shared._lyzn_kd_cleanup then
    pcall(shared._lyzn_kd_cleanup)
end
-- nuke any leftover GUIs from crashed runs
for _, loc in {game:GetService("CoreGui"), gethui and gethui() or nil} do
    if loc then
        for _, g in loc:GetChildren() do
            if g.Name == "LyznESP_KD" or g.Name == "LyznSkeleton_KD" then
                pcall(function() g:Destroy() end)
            end
            if g:IsA("ScreenGui") and g:FindFirstChild("Window") then
                local w = g:FindFirstChild("Window")
                if w and w:IsA("CanvasGroup") then
                    local sb = w:FindFirstChild("Sidebar")
                    if sb then pcall(function() g:Destroy() end) end
                end
            end
        end
    end
end
shared._lyzn_kd_stop = false
...
local function createESP(player)
    ...
    local function setup(char)
        ...
        local hrp = char:FindFirstChild("HumanoidRootPart") or char:FindFirstChild("Head") or char:FindFirstChildWhichIsA("BasePart")
        if not hrp then
            local waitConn
            waitConn = char.ChildAdded:Connect(function(child)
                if child:IsA("BasePart") and not espFolder:FindFirstChild(player.Name) then
                    waitConn:Disconnect()
                    task.spawn(setup, char)
                end
            end)
            table.insert(conns, waitConn)
            return
        end
        ...
        nameLabel.Position = UDim2.fromOffset(6, 4)
        ...
        hpBarBg.Size = UDim2.new(1, -12, 0, 3)
        hpBarBg.Position = UDim2.new(0, 6, 0, 22)
        hpBarBg.BackgroundColor3 = Color3.fromRGB(40, 40, 48)
        ...
        distLabel.Size = UDim2.new(1, -12, 0, 12)
        distLabel.Position = UDim2.new(0, 6, 0, 28)
        distLabel.BackgroundTransparency = 1
        distLabel.TextColor3 = Color3.fromRGB(160, 160, 175)
        ...
        hl.OutlineColor = Color3.fromRGB(248, 248, 252)
        hl.FillColor = Color3.fromRGB(248, 248, 252)
        ...
        for i = 1, #SkeletonBones do
            ...
            line.Name = "Bone" .. i
            line.BackgroundColor3 = Color3.fromRGB(248, 248, 252)
        ...
        local jointNames = {}
        for _, bone in SkeletonBones do
            jointNames[bone[1]] = true
            jointNames[bone[2]] = true
        end

        local jointDots = {}
        for name in jointNames do
        ...
        conn = RunService.RenderStepped:Connect(function()
            if shared._lyzn_kd_stop then conn:Disconnect() return end
            if not State.ESP then
                bill.Enabled = false
                hl.Enabled = false
                for _, line in boneLines do line.Visible = false end
                for _, dot in jointDots do dot.Visible = false end
                return
            end
            bill.Enabled = true
            hl.Enabled = true
            ...
            if myHRP and hrp and hrp.Parent then
                distLabel.Text = math.floor((myHRP.Position - hrp.Position).Magnitude) .. "m"
            end
            local hp, maxHp
            local attrHP = char:GetAttribute("HP")
            local attrMaxHP = char:GetAttribute("MaxHP")
            if attrHP and attrMaxHP then
                hp, maxHp = attrHP, attrMaxHP
            elseif hum then
                hp, maxHp = hum.Health, hum.MaxHealth
            end
            if hp and maxHp and maxHp > 0 then
                local ratio = math.clamp(hp / maxHp, 0, 1)
                nameLabel.Text = player.DisplayName .. "  " .. math.floor(hp) .. "hp"
                hpBarFill.Size = UDim2.fromScale(ratio, 1)
                hpBarFill.BackgroundColor3 = Color3.fromRGB(255 * (1 - ratio), 255 * ratio, 0)
                cardStroke.Color = Color3.fromRGB(180 + 75 * (1 - ratio), 180 + 75 * ratio, 180 * ratio)
                hl.OutlineColor = Color3.fromRGB(200 + 55 * (1 - ratio), 200 + 55 * ratio, 200 * ratio)
            else
            ...
            local boneColor = Color3.fromRGB(248, 248, 252)
            if hp and maxHp and maxHp > 0 then
                local ratio = math.clamp(hp / maxHp, 0, 1)
                boneColor = Color3.fromRGB(255 * (1 - ratio), 200 + 55 * ratio, 200 * ratio)
            end
            ...
            for name in jointNames do
                ...
                if part and part:IsA("BasePart") then
                    local sp, onScreen, depth = worldToScreen(part.Position)
                    if onScreen and depth > 0 then
                        screenPositions[name] = sp
                        local dot = jointDots[name]
                        dot.Position = UDim2.fromOffset(sp.X, sp.Y)
                        dot.BackgroundColor3 = boneColor
                        dot.Visible = true
                    else
                        jointDots[name].Visible = false
                    end
                else
                    jointDots[name].Visible = false
                end
            ...
            for i, bone in ipairs(SkeletonBones) do
                local p1 = screenPositions[bone[1]]
                local p2 = screenPositions[bone[2]]
                ...
                if p1 and p2 then
                    ...
                    line.Rotation = angle
                    line.BackgroundColor3 = boneColor
                    line.Visible = true
                else
                    line.Visible = false
                end
        ...
        local cleanupConn
        cleanupConn = char.AncestryChanged:Connect(function()
            if not char:IsDescendantOf(workspace) then
                cleanupConn:Disconnect()
                for _, line in boneLines do line:Destroy() end
                for _, dot in jointDots do dot:Destroy() end
            end
        end)
    end
    ...
    local conn = player.CharacterAdded:Connect(function(c)
        if shared._lyzn_kd_stop then return end
        task.wait(0.5)
        setup(c)
    end)
    table.insert(conns, conn)
...
local function refreshESP()
    ...
    for _, c in skeletonGui:GetChildren() do c:Destroy() end
    for _, p in Players:GetPlayers() do
        createESP(p)
    end
end
...
-- ══════════════════════════════════════
--  Speed Boost (MovementConfig + direct WalkSpeed)
-- ══════════════════════════════════════
local MovementConfig = nil
local origSprintSpeed = 26
local origMaxMomentum = 28
local origBaseWalkSpeed = 16

local function findMovementConfig()
    local ok, cfg = pcall(function()
        return require(LP.PlayerScripts.Controllers.Character.MovementController.MovementConfig)
    end)
    if ok and cfg then
        origSprintSpeed = cfg.SPRINT_SPEED or 26
        origMaxMomentum = cfg.MAX_MOMENTUM_SPEED or 28
        origBaseWalkSpeed = cfg.BASE_WALK_SPEED or 16
        return cfg
    end
    return nil
end
...
local function startSpeedBoost()
    MovementConfig = findMovementConfig()

    local conn
    conn = RunService.Heartbeat:Connect(function()
        if shared._lyzn_kd_stop then
            if MovementConfig then
                MovementConfig.SPRINT_SPEED = origSprintSpeed
                MovementConfig.MAX_MOMENTUM_SPEED = origMaxMomentum
                MovementConfig.BASE_WALK_SPEED = origBaseWalkSpeed
            end
            conn:Disconnect()
            return
        end
        if State.SpeedBoost then
            if MovementConfig then
                MovementConfig.SPRINT_SPEED = State.SpeedValue
                MovementConfig.MAX_MOMENTUM_SPEED = State.SpeedValue + 2
                MovementConfig.BASE_WALK_SPEED = State.SpeedValue
            end
            local hum = getHum()
            if hum then
...
-- ══════════════════════════════════════
--  Silent Aim (camera snap + FOV circle)
-- ══════════════════════════════════════
...
fovCircle.Color = Color3.fromRGB(248, 248, 252)
fovCircle.Visible = false

local aimSnapping = false

local function getAimPartFromChar(char)
    if State.AimTarget == "Head" then
        return char:FindFirstChild("Head")
    elseif State.AimTarget == "Legs" then
        return char:FindFirstChild("LeftUpperLeg") or char:FindFirstChild("RightUpperLeg") or char:FindFirstChild("HumanoidRootPart")
    else
        return char:FindFirstChild("UpperTorso") or char:FindFirstChild("HumanoidRootPart") or char:FindFirstChild("Head")
    end
end

local function startSilentAim()
    RunService:BindToRenderStep("LyznSilentAim", Enum.RenderPriority.Camera.Value + 1, function()
        if shared._lyzn_kd_stop then
            RunService:UnbindFromRenderStep("LyznSilentAim")
            fovCircle.Visible = false
            return
        end
        ...
        fovCircle.Visible = State.SilentAim and State.ShowFOV

        if not State.SilentAim or not aimSnapping then return end

        local target = getClosestPlayerInFOV()
        if target and target.Character then
            local aimPart = getAimPartFromChar(target.Character)
            if aimPart and cam then
                local origin = cam.CFrame.Position
                local dir = (aimPart.Position - origin).Unit
                cam.CFrame = CFrame.lookAt(origin, origin + dir)
            end
        end
    end)
    ...
    local mouseDown
    mouseDown = UserInputService.InputBegan:Connect(function(input, processed)
        if shared._lyzn_kd_stop then mouseDown:Disconnect() return end
    ...
    table.insert(conns, mouseDown)

    local mouseUp
    mouseUp = UserInputService.InputEnded:Connect(function(input)
        if shared._lyzn_kd_stop then mouseUp:Disconnect() return end
        if isThrowInput(input) then
            aimSnapping = false
        end
    end)
    table.insert(conns, mouseUp)

    -- Mobile: detect throw button press via MobileControlsController's touch
    local touchDown
    touchDown = UserInputService.TouchStarted:Connect(function(touch, processed)
        if shared._lyzn_kd_stop then touchDown:Disconnect() return end
    ...
    table.insert(conns, touchDown)

    local touchUp
    touchUp = UserInputService.TouchEnded:Connect(function()
        if shared._lyzn_kd_stop then touchUp:Disconnect() return end
        aimSnapping = false
    end)
    table.insert(conns, touchUp)
end
...
-- ══════════════════════════════════════
local expandedParts = {}
local hitboxBodyParts = {"Head", "UpperTorso", "LowerTorso", "LeftUpperArm", "RightUpperArm", "LeftUpperLeg", "RightUpperLeg"}

local function expandCharacter(char, multiplier)
    if not char then return end
    for _, partName in hitboxBodyParts do
        local part = char:FindFirstChild(partName)
        if part and part:IsA("BasePart") then
            local key = char:GetFullName() .. "/" .. partName
            if not expandedParts[key] then
                expandedParts[key] = part.Size
            end
            part.Size = expandedParts[key] * multiplier
...
local function resetCharacter(char)
    ...
    for _, partName in hitboxBodyParts do
        ...
        if part and part:IsA("BasePart") then
            local key = char:GetFullName() .. "/" .. partName
            if expandedParts[key] then
                part.Size = expandedParts[key]
                part.Transparency = 0
                expandedParts[key] = nil
            end
        end
    end
end
...
local function startHitboxExpander()
    ...
    conn = RunService.Heartbeat:Connect(function()
        ...
        for _, p in Players:GetPlayers() do
            if p ~= LP and p.Character and isAlive(p.Character) then
                local key = p.Character:GetFullName() .. "/Head"
                if not expandedParts[key] then
                    expandCharacter(p.Character, State.HitboxSize)
                end
            end
        end
    end)
...
local Window = Library:CreateWindow({ Title = "Lyzn Hub", SubTitle = "Knife Duels" })

-- Combat Tab
local CombatTab = Window:CreateTab("Combat")

local AimSec = CombatTab:CreateSection("Silent Aim", "Left")
AimSec:AddToggle({ Text = "Enable Silent Aim", Default = false, Callback = function(on) State.SilentAim = on end })
AimSec:AddSlider({ Text = "FOV Radius", Min = 50, Max = 400, Default = 150, Decimals = 0, Callback = function(v) State.FOVRadius = v end })
AimSec:AddToggle({ Text = "Show FOV Circle", Default = true, Callback = function(on) State.ShowFOV = on end })
AimSec:AddDropdown({ Text = "Aim Target", List = {"Head", "Body", "Legs"}, Default = "Body", Callback = function(v) State.AimTarget = v end })
AimSec:AddLabel("Snaps aim to closest player in FOV on click")

local HitboxSec = CombatTab:CreateSection("Hitbox Expander", "Right")
HitboxSec:AddToggle({ Text = "Enable Hitbox Expander", Default = false, Callback = function(on)
    State.HitboxExpand = on
    if not on then
        for _, p in Players:GetPlayers() do
            if p ~= LP and p.Character then resetCharacter(p.Character) end
        end
        expandedParts = {}
    end
end })
HitboxSec:AddSlider({ Text = "Size Multiplier", Min = 2, Max = 8, Default = 3, Decimals = 1, Callback = function(v)
    State.HitboxSize = v
    if State.HitboxExpand then
        for _, p in Players:GetPlayers() do
            if p ~= LP and p.Character then
                resetCharacter(p.Character)
                expandCharacter(p.Character, v)
            end
        end
    end
end })
HitboxSec:AddLabel("Expands hitbox")

-- Visuals Tab
local VisualsTab = Window:CreateTab("Visuals")

local EspSec = VisualsTab:CreateSection("ESP", "Left")
EspSec:AddToggle({ Text = "Enable ESP", Default = true, Callback = function(on)
    State.ESP = on
    if on then refreshESP() end
end })
EspSec:AddLabel("Skeleton + names + health + distance")

-- Movement Tab
local MoveTab = Window:CreateTab("Movement")

local SpeedSec = MoveTab:CreateSection("Speed", "Left")
SpeedSec:AddToggle({ Text = "Speed Boost", Default = false, Callback = function(on) State.SpeedBoost = on end })
SpeedSec:AddSlider({ Text = "Speed", Min = 16, Max = 50, Default = 32, Decimals = 0, Callback = function(v) State.SpeedValue = v end })
SpeedSec:AddLabel("Sets WalkSpeed")

-- Misc Tab
local MiscTab = Window:CreateTab("Misc")

local InfoSec = MiscTab:CreateSection("Info", "Left")
InfoSec:AddLabel("LCtrl / DPadUp / L button = Toggle UI")
InfoSec:AddLabel("End = Destroy Script")
InfoSec:AddLabel("Knife Duels")
...
Players.PlayerAdded:Connect(function(p)
    if shared._lyzn_kd_stop then return end
    task.wait(1)
    if State.ESP and not espFolder:FindFirstChild(p.Name) then
        createESP(p)
    end
end)
Players.PlayerRemoving:Connect(function(p)
    local f = espFolder:FindFirstChild(p.Name)
    if f then f:Destroy() end
end)

task.spawn(function()
    while not shared._lyzn_kd_stop do
        task.wait(3)
        if State.ESP then
            for _, p in Players:GetPlayers() do
